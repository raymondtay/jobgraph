...
class DataflowRunnerSpecs extends Specification with ScalaCheck {
  ...
  def verifyCanRunDataflowRunnerAndReturnAsString = {
    ...
    val ctx =
      Context(getClass.getClassLoader.getResource("gcloud_monitor_job.sh").getPath.toString :: Nil, fakeJobId, "")
  ...
  def verifyCanRunDataflowRunnerAndReturnAsJson = {
    ...
    val fakeJobId = "TEST12334341241351241241243213213123213123"
    val ctx =
      Context(getClass.getClassLoader.getResource("gcloud_monitor_job2.sh").getPath.toString :: Nil, fakeJobId, _root_.io.circe.Json.Null)
    val runner = new DataflowRunner
  ...
  def verifyCanRunDataflowRunnerAndReturnAsJson2 = {
    ...
    val fakeJobId = "TEST12334341241351241241243213213123213123"
    val ctx =
      Context(getClass.getClassLoader.getResource("gcloud_monitor_job3.sh").getPath.toString :: Nil, fakeJobId, _root_.io.circe.Json.Null)
    val runner = new DataflowRunner
    val result = runner.run(ctx)(jsonParser.parse)
